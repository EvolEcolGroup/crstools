...
2) `inspect_crs` visualises the distortion associated with a given crs, by
plotting a grid of lines that show how the CRS distorts the map as well as the
Tissot indicatrix (the distortion of circles on the map).
...
```{r}
terra::crs(s_america_r)
```
which gives a WKT.

We can ask for the "Proj4" string with:
```{r}
terra::crs(s_america_r, proj = TRUE)
```
...
Let's use it:
```{r plot_raster_europe_equal_area}
ggplot() +
  geom_spatraster(data = europe_r) +
  scale_fill_viridis_c(na.value = "transparent") +
