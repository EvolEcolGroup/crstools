# Contributing to crstools
This document outlines how to contribute to the development of `crstools`. The
package is maintained on a voluntary basis, so help is always appreciated.

## The basic process of contributing
Development work for `crstools` occurs in the `dev` branch.
...
whether you should add an appropriate unit test. `crstools` uses the `testthat`
...
## Functional changes
If you want to make a change that impacts the functioning of `crstools`, it's a
