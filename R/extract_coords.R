...
extract_coords <- function(georef_image, coords_df = NULL, col = "red") {
  ...
  if (!is.null(coords_df) && !is.data.frame(coords_df)) {
    stop(
      "coords_df must be a dataframe with columns id, longitude, and latitude."
    )
  }
  if (!is.null(coords_df)) {
    ...
    if (!all(required_cols %in% colnames(coords_df))) {
      stop(
        "coords_df dataframe must contain columns: ",
  ...
  plot(
    0,
    0,
    xlim = c(0, dim(georef_image)[1]),
    ylim = c(0, dim(georef_image)[2]),
    type = "n",
    xlab = "x_pixels",
    ylab = "y_pixels"
  )
