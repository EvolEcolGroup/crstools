...
#' @param distortion The distortion of the world map projection
#' (e.g., "equal_area", "equidistant", "conformal").
...
crs_square_format <- function(distortion, center,
  ...
  if (center$lat > 75) {
    ...
    if (distortion == "conformal") {
      crs_suggestions <- data.frame(
        prj = "stere", x0 = NA_real_, lat0 = 0, lat1 = NA_real_,
        lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
      ...
      crs_suggestions <- data.frame(
        prj = "laea", x0 = NA_real_, lat0 = 0, lat1 = NA_real_,
        lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
    ...
    if (distortion == "conformal") {
      crs_suggestions <- data.frame(
        prj = "stere", x0 = NA_real_, lat0 = center$lat, lat1 = NA_real_,
        lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
  ...
  if (distortion == "conformal" && !quiet) {
    message(paste0(
      ...
      "one can also apply a scale factor. Various values for can ",
      "be applied and the area distortion patterns along ",
      "the center and at the border of the map are compared to ",
      "select most appropriate value."
    ))
  }
