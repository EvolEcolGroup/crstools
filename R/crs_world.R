...
#' (e.g., "equal_area", "equidistant", "compromise").
...
crs_world <- function(distortion, center, scale, round_cm, world_equidist,
  ...
  crs_suggestions <- data.frame(
    prj = character(), x0 = double(), lat0 = double(), lat1 = double(),
    lat2 = double(), lon0 = double(), k0 = double(),
  ...
  if (distortion == "equal_area") {
    ...
    for (i in 3:6) {
      ...
      crs_suggestions <- rbind(
        crs_suggestions,
        data.frame(
          prj = proj4, x0 = NA_real_, lat0 = NA_real_, lat1 = NA_real_,
          lat2 = NA_real_, lon0 = lng, k0 = NA_real_,
          description = projection_name,
          notes = paste0(
            "Equal-area world map projection ",
            "with poles represented as lines"
          )
        )
      )
    }
    ...
    for (i in 1:2) {
      ...
      crs_suggestions <- rbind(
        ...
        data.frame(
          ...
          notes = paste0(
            "Equal-area world map projection ",
            "with poles represented as points"
          )
        )
      )
    }
    ...
    if (is.null(world_equidist)) {
      stop(paste0(
        "`world_equidist` must be provided ",
        "for equidistant world map projections"
      ))
    }
