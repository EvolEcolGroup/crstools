...
crs_small_area <- function(distortion, center, scale,
  ...
  if (distortion == "equidistant") {
    ...
    if (center$lat > 70) {
      ...
      # ))
      crs_suggestions <- data.frame(
        prj = "cass", x0 = NA_real_, lat0 = 0, lat1 = NA_real_, lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
    ...
    if (ratio > 1.25) {
      ...
      crs_suggestions <- crs_ew_extent(distortion, center, scale, lonmin = lonmin, lonmax = lonmax, latmin = latmin, latmax = latmax)
    } else {
