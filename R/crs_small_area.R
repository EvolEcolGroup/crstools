...
#' "conformal").
#' @param center data.frame with two numeric values, latitude and longitude
  ...
  if (distortion == "equidistant") {
    if (center$lat > 70) {
      crs_suggestions <- data.frame(
        prj = "aeqd", x0 = NA_real_, lat0 = 90, lat1 = NA_real_,
        lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
        description = "Polar azimuthal equidistant",
        notes = paste0(
          "Distance correct along ",
          "any line passing through the pole (i.e., meridian)"
        )
      )
      ...
      crs_suggestions <- data.frame(
        prj = "cass", x0 = NA_real_, lat0 = 0, lat1 = NA_real_,
        lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
        description = "Cassini",
        notes = paste0(
          "Distance correct along ",
          "any line perpendicular to the central meridian"
        )
      )
    } else if (abs(center$lat) < 15) {
      lat_s <- ifelse(latmax * latmin <= 0,
        max(abs(latmax), abs(latmin)) / 2, center$lat
      )
      crs_suggestions <- data.frame(
        prj = "eqc", x0 = NA_real_, lat0 = NA_real_, lat1 = lat_s,
        lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
      ...
      crs_suggestions <- data.frame(
        prj = "eqdc", x0 = NA_real_, lat0 = center$lat,
      ...
      crs_suggestions <- rbind(
        crs_suggestions,
        data.frame(
          prj = "aeqd", x0 = NA_real_, lat0 = center$lat, lat1 = NA_real_,
          lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
          description = "Azimuthal equidistant",
          notes = paste0(
            "distance correct along ",
            "any line passing through the center of the map ",
            "(i.e., great circle)"
          )
        )
      )
    }
  } else if ((latmin >= 84 && distortion == "conformal") ||
    ...
    scale_factor <- 0.994
    crs_suggestions <- data.frame(
      prj = "stere", x0 = NA_real_, lat0 = preview_map_lat_0, lat1 = NA_real_,
      lat2 = NA_real_, lon0 = center$lng, k0 = scale_factor,
    ...
    scale_factor <- 0.9999
    crs_suggestions <- data.frame(
      prj = "tmerc", x0 = 500000, lat0 = preview_map_lat_0, lat1 = NA_real_,
      lat2 = NA_real_, lon0 = center$lng, k0 = scale_factor,
    ...
    if (ratio > 1.25) {
      ...
      crs_suggestions <- crs_ew_extent(distortion, center, scale,
