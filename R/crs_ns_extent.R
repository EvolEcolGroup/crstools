...
crs_ns_extent <- function(distortion, center, quiet = FALSE) {
  ...
  if (distortion == "conformal") {
    ...
    crs_suggestions <- data.frame(
      ...
      notes = "Equal-area projection for regional maps with a north-south extent"
    )
  }
  ...
  if (distortion == "equal_area" && !quiet) {
    message(
      "To reduce overall distortion on the map, one can also compress the map in the north-south direction (with a factor s) and expand the map in the east-west direction (with a factor 1 / s). The factor s can be determined with a trial-and-error approach, comparing the distortion patterns along the center and at the border of the map."
    )
  } else if (distortion == "conformal" && !quiet) {
    message(
      "To reduce overall area distortion on the map, one can also apply a scale factor k. Various values for k can be applied and the area distortion patterns along the center and at the border of the map are compared to select most appropriate value."
    )
  }
