...
crs_ns_extent <- function(distortion, center, quiet = FALSE) {
  ...
  if (distortion == "conformal") {
    crs_suggestions <- data.frame(
      prj = "tmerc", x0 = NA_real_, lat0 = NA_real_, lat1 = NA_real_,
      lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
      description = "Transverse Mercator",
      notes = paste0(
        "Conformal projection ",
        "for regional maps with a north-south extent"
      )
    )
  } else if (distortion == "equal_area") {
    crs_suggestions <- data.frame(
      prj = "tcea", x0 = NA_real_, lat0 = NA_real_, lat1 = NA_real_,
      lat2 = NA_real_, lon0 = center$lng, k0 = NA_real_,
  ...
  if (distortion == "equal_area" && !quiet) {
    message(paste0(
      "To reduce overall distortion on the map, one can also ",
      "compress the map in the north-south direction (with a factor s) and ",
      ...
      "comparing the distortion patterns along the center, ",
      "and at the border of the map."
    ))
    ...
    message(paste0(
      ...
      "the area distortion patterns along the center and at the border of the ",
      "map are compared to select most appropriate value."
    ))
  }
